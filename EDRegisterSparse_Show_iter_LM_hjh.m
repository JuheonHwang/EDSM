...
function P0 = runOptim(P0, iteration, SrcMesh, pntTar, faceTar, G0, nodeIdx, idxNeighbor, idxNodeNeighbor, distNeighbor, Lap, w_rot, w_smooth, w_lap, R_, T_, A_, t_, show_iteration, show_final)
    ...
    values = struct('Jsmooth', []);

    for i = 1:iteration

        [F, P0, Gradient, delta, lambda, values] = EfuncLocal(F, P0, lambda, values, SrcMesh, pntTar, G0, nodeIdx, idxNeighbor, idxNodeNeighbor, distNeighbor, Lap, w_rot, w_smooth, w_lap, R_, T_, A_, t_, indices);
...
function [F_new, P0, Gradient, delta, lambda, values] = EfuncLocal(F_old, P0, lambda, values, SrcMesh, pntTar, G0, nodeIdx, idxNeighbor, idxNodeNeighbor, distNeighbor, Lap, w_rot, w_smooth, w_lap, R_, T_, A_, t_, indices)
    ...
    if isempty(F_old)
        ...
        if isempty(values.Jsmooth)
            ...
            values.Jsmooth = w_smooth * Jsmooth  ./ sqrt(3 * n_nodes);
        end
        ...
        values.v1 = v1;
        values.deformedV = deformedV;
        values.deformedV_local = deformedV_local;
    else
        Efit = values.Efit;
        Erot1 = values.Erot1;
        Erot2 = values.Erot2;
        ...
        v1 = values.v1;
        deformedV = values.deformedV;
        deformedV_local = values.deformedV_local;
        Jsmooth = values.Jsmooth;
    ...
    if nargout > 1
        ...
        deformedV_R_col = reshape(repmat(1:9, [n_verts 1]), [], 1);
        deformedV_R_row = repmat((1:3*n_verts)', 3, 1);
        deformedV_R_val = reshape(repmat(gather(deformedV_local), 3, 1), [], 1);
        ...
        J = [Jfit ./ sqrt(n_target);
            ...
            Jsmooth;
            w_lap * Jlap ./ reshape(repmat(full(gather(diag(Lap))), [3 1]), [], 1)];
        ...
        for i = 1:3
            Jp = J' * J;
            % delta = -(inv(Jp + lambda * eye(length(Jp))) * J' * F_old);
            delta = -(inv(Jp + lambda * diag(diag(Jp))) * J' * F_old);
            P0_new = P0 + delta;
            F_new = EfuncLocal([], P0_new, lambda, values, SrcMesh, pntTar, G0, nodeIdx, idxNeighbor, idxNodeNeighbor, distNeighbor, Lap, w_rot, w_smooth, w_lap, R_, T_, A_, t_, []);
            if (sum(F_new.^2) < sum(F_old.^2)) && (i == 1)
                lambda = lambda * 0.1;
                P0 = P0_new;
                break
            else
