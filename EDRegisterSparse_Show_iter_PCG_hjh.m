...
P0 = runOptim(P0, iteration, gpuSrcMesh, pntTar, TarMesh.Fpoi, gpuArray(G0), nodeIdx, idxNeighbor, idxNodeNeighbor, distNeighbor, gpuArray(Lap), w_rot, w_smooth, w_lap, R0, T0, A0, t0, mode, show_iteration, show_final);
...
function P0 = runOptim(P0, iteration, SrcMesh, pntTar, faceTar, G0, nodeIdx, idxNeighbor, idxNodeNeighbor, distNeighbor, Lap, w_rot, w_smooth, w_lap, R_, T_, A_, t_, mode, show_iteration, show_final)
    tic;
    epsilon = 0.000001;
    epsilon2 = 0.001;
    epsilon3 = 0.01;
    ...
    for i = 1:iteration
        lambda_in = lambda;
        [F, J, P0, lambda, Gradient, delta] = EfuncLocal(F, J, P0, lambda, SrcMesh, pntTar, G0, nodeIdx, idxNeighbor, idxNodeNeighbor, distNeighbor, Lap, w_rot, w_smooth, w_lap, R_, T_, A_, t_, mode);
        
        if lambda > 100000
            break
        end

        if (lambda_in / lambda > 1)
            used_lambda = lambda_in;
        else
            used_lambda = lambda;
        end

        fprintf("Iter %d Error : %f, lambda : %f \n", i, sum(F.^2), used_lambda);
...
function [F_new, J_new, P0, lambda, Gradient, delta] = EfuncLocal(F_old, J_old, P0, lambda, SrcMesh, pntTar, G0, nodeIdx, idxNeighbor, idxNodeNeighbor, distNeighbor, Lap, w_rot, w_smooth, w_lap, R_, T_, A_, t_, mode, indices)  
    if isempty(F_old) && isempty(J_old)
        ...
        F_old = [Efit(:); w_rot*Erot1(:); w_rot*Erot2(:); w_smooth*Esmooth(:); w_lap* Elap(:)];
    ...
    if nargout == 2
        ...
        switch mode
            case 'none'
                A = Jp;
            case 'eye'
                A = Jp + lambda * eye(length(Jp));
            case 'diag'
                A = Jp + lambda * diag(diag(Jp));
        end
        
        
        lambda_decrease = true;

        while true
            delta = zeros(size(P0));
            [delta, stats] = preconjgrad(A, -b, delta);
            
            P0_new = P0 + delta;
            [F_new, J_new] = EfuncLocal([], [], P0_new, lambda, SrcMesh, pntTar, G0, nodeIdx, idxNeighbor, idxNodeNeighbor, distNeighbor, Lap, w_rot, w_smooth, w_lap, R_, T_, A_, t_);
            if sum(F_new(:).^2) < sum(F_old(:).^2)
                if lambda_decrease
                    lambda = lambda / 10;
                end
                P0 = P0_new;
...
function [delta, stats] = preconjgrad(A, b, delta)
    Precond = diag(A);
    %Precond = ones(size(A, 1), 1);

    % M = Precond' * Precond;
    %inv_M = inv(Precond' * Precond);
    ...
    for i = 1:length(b)
        ...
        r_new = residual' * residual;
        stats = [stats; sqrt(r_new)]; %#ok<*AGROW> 
        if sqrt(r_new) < 1e-10
            break
        end
        % z = M\residual;
...
function delta = conjgrad(A, b, delta)
    residual = b - A * delta;
    p = residual;
    r_old = residual' * residual;
