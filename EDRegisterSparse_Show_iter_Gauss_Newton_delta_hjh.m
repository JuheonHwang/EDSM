...
function P0 = runOptim(P0, iteration, SrcMesh, pntTar, faceTar, G0, nodeIdx, idxNeighbor, idxNodeNeighbor, distNeighbor, Lap, w_rot, w_smooth, w_lap, R_, T_, A_, t_, show_iteration, show_final)
    ...
    for i = 1:iteration
        ...
        [F, P0, Gradient, delta] = EfuncLocal(P0, SrcMesh, pntTar, G0, nodeIdx, idxNeighbor, idxNodeNeighbor, distNeighbor, Lap, w_rot, w_smooth, w_lap, R_, T_, A_, t_);
...
function [F, P0, Gradient, delta] = EfuncLocal(P0, SrcMesh, pntTar, G0, nodeIdx, idxNeighbor, idxNodeNeighbor, distNeighbor, Lap, w_rot, w_smooth, w_lap, R_, T_, A_, t_, indices)  
...
%     deformedV_R_row = reshape(repmat(1:3*n_verts, [1 3]), [], 1);
...
%         w_lap * Jlap ./ reshape(repmat(full(gather(diag(Lap))), [3 1]), [], 1)]; %% JW FIXED NEEDED CHECK
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Finite-Diff%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if nargout > 1
        jacobian_delta = 0.00001;
        J = zeros(numel(F), numel(P0));
%         idx = 1:numel(P0);
%         for i = 1:length(idx)
%             Pt = P0;
...
%         end
        ...
        J((1 + 3 * n_target):(3 * 3 * n_nodes + 3 * n_target), :) = w_rot *  J((1 + 3 * n_target):(3 * 3 * n_nodes + 3 * n_target), :) ./ sqrt(n_nodes);
        J((1 + 3 * 3 * n_nodes + 3 * n_target):(n_nodes + 3 * 3 * n_nodes + 3 * n_target), :) = w_rot *  J((1 + 3 * 3 * n_nodes + 3 * n_target):(n_nodes + 3 * 3 * n_nodes + 3 * n_target), :) ./ sqrt(n_nodes);
        J((1 + n_nodes + 3 * 3 * n_nodes + 3 * n_target):(3 * n_nodes + n_nodes + 3 * 3 * n_nodes + 3 * n_target), :) = w_smooth * J((1 + n_nodes + 3 * 3 * n_nodes + 3 * n_target):(3 * n_nodes + n_nodes + 3 * 3 * n_nodes + 3 * n_target), :) ./ sqrt(3 * n_nodes);
        J((1 + 3 * n_nodes + n_nodes + 3 * 3 * n_nodes + 3 * n_target):end, :) = w_lap * J((1 + 3 * n_nodes + n_nodes + 3 * 3 * n_nodes + 3 * n_target):end, :) ./ reshape(repmat(full(diag(gather(Lap))), [3 1]), [], 1);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Gauss-Newton%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        Gradient = 2 * (F' * J)';
        % delta = -(inv(J'*J)*J' * F);
        % delta = -((J'*J)\J' * F);
